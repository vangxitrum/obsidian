...
The mobile app uses Expo SDK 54, React Native, Expo Router, and TypeScript under `apps/mobile`. The warm editorial design uses Lora headings, Be Vietnam Pro UI text, cream surfaces, plum primary actions, and terracotta/sage/gold accents. Phase 1 mobile now has Supabase email OTP auth with SecureStore-backed sessions, bearer-authenticated Go API queries/mutations, and Expo notification registration plus a 60-day local schedule reconciliation path. An explicit persisted `Xem bản mẫu` mode keeps local demo data available without silently masking production errors. Supabase PostgreSQL is configured locally through an ignored environment file, while production auth and push still require publishable/EAS credentials.
